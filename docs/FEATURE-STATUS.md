...
| **The original logo artwork** | A pixel-exact brand | The mark in the application and in `build/icon.svg` is a vector *reconstruction* of the supplied logo, not the supplied file itself. It is used everywhere the brand appears and generates the app icon; swapping in the original is documented in [PACKAGING.md](PACKAGING.md) §6. |
